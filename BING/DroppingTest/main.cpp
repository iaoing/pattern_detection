...
int main()
{
	...
	fd = fopen("dropping.index.03","rb");
	assert(fd != NULL);
	fseek(fd, 0, SEEK_END);
	flen = ftell(fd);
	rewind(fd);

	ibuf = (char*)malloc(sizeof(char)*flen);
	assert(ibuf != NULL);

	result = fread(ibuf, 1, flen, fd);
	assert(result == flen);

	string str ;
	str.resize(flen);
	memcpy(&str[0], &ibuf[0], flen);

	cout << str << endl;


	cout << "\n\n\nPatternDeSe.h DeSerialization:\n"
         << str << endl;
    void *patbuf = (void*)&str[0];
	PatternListDeSe pat_List_DeSe;
	pat_List_DeSe.Pattern_DeSe(patbuf, flen);
	pat_List_DeSe.show();

	cout << "\n\n\nPatternDeSe.h analyze_unit:\n"
         << str << endl;
	...
	{
		pat_unit.get_ibuf_unit(it->second.dese_unit);
		pat_unit.show();
	}
	

//	PatternForQuery pfq;
	//int ret = pat_List_DeSe.dese_map_look_up(0, &pfq);
//	pfq.show();

	//for(it = pat_List_DeSe.dese_map.begin(); it != pat_List_DeSe.dese_map.end(); ++it)
	//{
	//	for(PatternForQuery p : it->second.queryList)
	//		p.show();
	//}

	// cout << "\n\n\nInit the index info for test!:\n" << endl;
	// init_writebuf();
	// // show_writebuf();
	

	// cout << "\n\n\nPatternIndex Detection:\n" << endl;
 ...
 //    cout << "\n\n\nPatternIndex Serialization:\n"
 //         << buf << endl;

 //    p_index.~PatternIndex();
	

	// cout << "\n\n\nPatternIndex DeSerialization:\n"
 //         << buf << endl;
